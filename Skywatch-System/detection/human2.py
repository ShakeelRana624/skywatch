...
class ActivityClassifier:
    ...
    def classify_single_frame(self, track_id, keypoints):
        ...
        if shoulder_visible and hip_visible and not knee_visible:
            ...
            if body_height < 60:  # Sitting position
                return "Sitting"
            else:
                return "Standing"
        
        # Knee-based posture detection
        if shoulder_visible and hip_visible and knee_visible and not ankle_visible:
            # Use knee angle to determine posture
            if avg_knee_angle < 60:  # Straight knees = standing
                return "Standing"
            else:  # Bent knees = sitting
                return "Sitting"
        ...
        if shoulder_visible and hip_visible and knee_visible:
            ...
            # Check if person is sitting first
            if body_height < 60:  # Sitting position
                return "Sitting"
            
            # Walking/Running detection
            if 2 <= velocity <= 6:  # Walking speed
                print(f"DEBUG: Walking detected - Velocity: {velocity:.1f}")
                return "Walking"
            elif velocity > 6:  # Running speed
                print(f"DEBUG: Running detected - Velocity: {velocity:.1f}")
                return "Running"
        
        # Remove duplicate walking/running detection - handled above
        
        # Final logic - check basic posture if nothing else detected
        if body_height < 60:  # Sitting position
            return "Sitting"
        else:
            return "Standing"
...
class HumanTracker:
    ...
    def draw_tracking_info(self, frame: np.ndarray, detections: List[Dict[str, Any]]) -> np.ndarray:
        ...
        # Draw text
        cv2.putText(annotated, f"Current: {current_count}", (20, 40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 2)
        cv2.putText(annotated, f"Unique: {unique_count}", (20, 70), 
        ...
        for detection in detections:
            ...
            # Draw ID and activity label - smaller text
            if is_occluded:
                label = f"P{track_id} - OCCL - {activity}"
                # Red tint for occluded
                label_color = (0, 0, 255)
            else:
                label = f"P{track_id} - {activity}"
                label_color = color
            ...
            if len(keypoints) >= 17:
                ...
                try:
                    ...
                    if all(len(p) > 0 for p in [left_shoulder, left_elbow, left_wrist]):
                        left_arm_angle = self.calculate_angle(left_elbow, left_shoulder, left_wrist)
                    else:
                        left_arm_angle = 0
                        
                    if all(len(p) > 0 for p in [right_shoulder, right_elbow, right_wrist]):
                        right_arm_angle = self.calculate_angle(right_elbow, right_shoulder, right_wrist)
                    else:
                        right_arm_angle = 0
                    
                    # Calculate leg angles
                    if all(len(p) > 0 for p in [left_hip, left_knee, left_ankle]):
                        left_leg_angle = self.calculate_angle(left_knee, left_hip, left_ankle)
                    else:
                        left_leg_angle = 0
                        
                    if all(len(p) > 0 for p in [right_hip, right_knee, right_ankle]):
                        right_leg_angle = self.calculate_angle(right_knee, right_hip, right_ankle)
                    else:
                        right_leg_angle = 0
                    ...
                    # Display angles near person - smaller text
                    angle_text = f"L:{left_arm_angle:.0f}° R:{right_arm_angle:.0f}°"
                    cv2.putText(annotated, angle_text, (x1, y1 - 25), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)
                    
                    height_text = f"H:{body_height:.0f}px"
                    cv2.putText(annotated, height_text, (x1, y2 + 15), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)
                    
                except:
                    pass  # Skip if angle calculation fails
